...
import logging
from typing import Callable, ClassVar, List, Generic, TypeVar, Dict, Set
...
class Reactive(Generic[T]):
    ...
    def listen(
        self, callback: Callable[[], None], auto_dispose: bool = True
    ) -> Observer:
        """
    ...
    def dispose(self):
        ...
        # Clean up dependency observers
        for observer in self._dep_observers:
            observer.dispose()
        self._dep_observers.clear()
...
class Computed(Reactive[T]):
    ...
    def __init__(
        self, compute_fn: Callable[[], T], dependencies: List[Reactive] = None
    ):
        """
        Args:
        ...
        self._dep_observers: List[Observer] = []  # Observers for dependencies

        # Subscribing to dependencies
        for dep in self._dependencies:
            self.logger.debug(f"Subscribing to dependency: {dep.__class__.__name__}")
            observer = dep.listen(self._update_value)
            self._dep_observers.append(observer)
